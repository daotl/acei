...
ACEI applications can run either within the _same_ process as the Tendermint
state-machine replication engine, or as a _separate_ process from the state-machine
replication engine. When run within the same process, Tendermint will call the ACEI
application methods directly as Go method calls.

When Tendermint and the ACEI application are run as separate processes, Tendermint
opens four connections to the application for ACEI methods. The connections each
handle a subset of the ACEI method calls. These subsets are defined as follows:
...
The `CheckTx` ACEI method controls what transactions are considered for inclusion in a block.
...
The `DeliverTx` ACEI method delivers transactions from Tendermint to the application.
...
field in their `Response*`. Applications may respond to these ACEI methods with a set of events.
Events allow applications to associate metadata about ACEI method execution with the
...
```go
 acei.ResponseDeliverTx{
  // ...
 Events: []acei.Event{
  {
   Type: "validator.provisions",
   Attributes: []acei.EventAttribute{
    acei.EventAttribute{Key: []byte("address"), Value: []byte("..."), Index: true},
    acei.EventAttribute{Key: []byte("amount"), Value: []byte("..."), Index: true},
    acei.EventAttribute{Key: []byte("balance"), Value: []byte("..."), Index: true},
   },
  },
  {
   Type: "validator.provisions",
   Attributes: []acei.EventAttribute{
    acei.EventAttribute{Key: []byte("address"), Value: []byte("..."), Index: true},
    acei.EventAttribute{Key: []byte("amount"), Value: []byte("..."), Index: false},
    acei.EventAttribute{Key: []byte("balance"), Value: []byte("..."), Index: false},
   },
  },
  {
   Type: "validator.slashed",
   Attributes: []acei.EventAttribute{
    acei.EventAttribute{Key: []byte("address"), Value: []byte("..."), Index: false},
    acei.EventAttribute{Key: []byte("amount"), Value: []byte("..."), Index: true},
    acei.EventAttribute{Key: []byte("reason"), Value: []byte("..."), Index: true},
   },
  },
  // ...
 },
...
ACEI applications must implement deterministic finite-state machines to be
...
external user or process except via the ACEI connections to a consensus engine
...
* **Usage**:
    * Echo a string to test an acei client/server implementation
...
* **Usage**:
    ...
    * `consensus_param_updates` returned for block `H` apply to the consensus
      ...
      see the [application spec entry on consensus parameters](../spec/acei/apps.md#consensus-parameters).
...
* **Usage**:
    ...
    * When all chunks have been accepted, Tendermint will make an ACEI `Info` call to verify that
...
Most of the data structures used in ACEI are shared [common data structures](../spec/core/data_structures.md). In certain cases, ACEI uses different data structures which are documented here:
...
* **Fields**:
    ...
    | version   | [VersionsParams](../core/data_structures.md#versionparams)       | The ACEI application version.                                                | 4            |
